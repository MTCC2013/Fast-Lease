...
public class ReservaEditar extends javax.swing.JFrame {
    ...
    int idUsuario;
    ReservaTableModel modelo;

    public ReservaEditar(int selecionado, int cdModelo, ReservaTableModel mtb) {
        initComponents();
        ...
        r = rc.buscarReservabyId(cdModelo);
        
        campoCod.setText(String.valueOf(r.getId()));
        
        Date dataTemp = new Date();
        SimpleDateFormat formataData = new SimpleDateFormat("dd/MM/yyyy");
        String data = formataData.format(r.getDtCadastro());
        campoDataCadastro.setText(data);
        SimpleDateFormat formataDat = new SimpleDateFormat("dd/MM/yyyy");
        String dat = formataDat.format(r.getDtCadastro());
        campoDataReserva.setText(dat);
        

        if (r.getTurno().equals("Matutino")) {
            cbTurno.setSelectedIndex(0);
        }
        if (r.getTurno().equals("Vespertino")) {
            cbTurno.setSelectedIndex(1);
        }
        if (r.getTurno().equals("Noturno")) {
            cbTurno.setSelectedIndex(2);
        }

        carregarComboSala(r.getIdSala().getId());
        carregarComboUsuario(r.getIdUsuario().getId());
        carregarComboCurso(r.getIdCurso().getId());
    ...
    private void initComponents() {
        ...
        labelCadCliente1.setText("ATUALIZAÇÃO DE RESERVA");
        ...
        painelCabecalhoCliente1Layout.setHorizontalGroup(
            ...
            .addGroup(painelCabecalhoCliente1Layout.createSequentialGroup()
                ...
                .addPreferredGap(javax.swing.LayoutStyle.ComponentPlacement.UNRELATED)
                .addComponent(btCadastroFun, javax.swing.GroupLayout.DEFAULT_SIZE, 91, Short.MAX_VALUE)
                .addContainerGap())
        );
...
//Validações
        Reserva r = new Reserva();
        r.setId(Integer.parseInt(campoCod.getText()));
        try {
        ...
        ReservaController rc = new ReservaController();
        rc.atualizar(r);
        modelo.updateReserva(linhaSelecionada,r);
        dispose();
    ...
    // Método que carrega combo da classe Sala

    private void carregarComboSala(int cdSala) {
        ...
        for (int linha = 0; linha < salas.size(); linha++) {
            //pegando a categoria da lista
            Sala sala = salas.get(linha);
            //adicionando a categoria no combo
            comboModel.addElement(sala);
            if (sala.getId() == cdSala) {
                comboModel.setSelectedItem(sala);
            }
    ...
    private void carregarComboUsuario(int cd) {
        //assim como é feito numa JTable, temos o DefaultComboBoxModel que é o model do JComboBox
        ...
        for (int linha = 0; linha < usuarios.size(); linha++) {
            ...
            if (usuario.getId() == cd) {
                comboModel.setSelectedItem(usuario);
            }

        }

    }
    private void carregarComboCurso(int cdCurso) {
         //assim como é feito numa JTable, temos o DefaultComboBoxModel que é o model do JComboBox
        ...
        for (int linha = 0; linha < cursos.size(); linha++) {
            ...
            if ( curso.getId() == cdCurso){
                comboModel.setSelectedItem(curso);
            }
